...
| 3.5 🟡 | **Port Metal** | Kernel densi validati su M1 Max (`--metal-dense-selftest`): matvec F32/Q8_0, RoPE NEOX, SwiGLU, RMSNorm, **blocco FFN**, **blocco attention GQA**. Resta: wiring pesi reali + KV + greedy | **Tutti i sub-blocchi PASS** (<1e-4); resto: greedy vs llama.cpp | Sì (Metal) |
